...
export default function FilterDualSlider({ filterName }: IFilterDualSliderProps) {
  ...
  const minInputChangeHandler = (event: FormEvent<HTMLInputElement>) => {
  ...
  const maxInputChangeHandler = (event: FormEvent<HTMLInputElement>) => {
    const target = event.target as HTMLInputElement;
  ...
  return (
      <div className="dual-slider">
        ...
        {
          !!filteredProducts.length && <div className="multi-range">
            <input
              type="range"
              min={0}
              max={items.length - 1}
              value={minVal}
              onInput={minInputChangeHandler}
